...
| Adds `bee_retrieval_preferred_candidates_selected`, flights that began with at least one preferred candidate, counted once per flight rather than once per attempt. It exists because no existing counter can say whether the preferred set reached a fetch: `preferred_attempts` increments only after the peer can be credited, and attempts plus overdrafts is inflated by the readmit path, which keeps a peer without consuming the candidate. | `main` | | [#299](https://github.com/crtahlin/wasp/issues/299) |

...
- **Compared with:** Bee **v2.8.2**, the latest released version of upstream Bee
  on 2026-09-10.
- **wasp described:** commit `467e30f0`, 2026-09-19. The latest wasp release is
  v0.1.3.
...
| Adds `bee_providers_discoveries_started`, `bee_providers_lookups_completed`, `bee_providers_lookups_served_from_cache`, `bee_providers_lookups_cancelled`, `bee_providers_connects_dialled`, `bee_providers_connects_already_connected` and `bee_providers_connects_failed`. Before these the package had no metrics and no success logging, so a working provider lookup and one that never ran were indistinguishable from outside the node. `connects_dialled` counts a connect procedure running to completion and does **not** prove a new connection was opened, because libp2p's already-connected short-circuit is keyed on the remote address rather than the peer. | `main` | | [#369](https://github.com/crtahlin/wasp/issues/369) |
| Provider discovery and hinted connection keep running after the request that started them ends, bounded by a 30 second timeout and still stopped by shutdown. Upstream has no such feature; in wasp they previously inherited the request's context, so a discovery lookup was cancelled when the download finished and, on erasure-coded content, within milliseconds of starting. The visible cost is that a node can hold a connection made for a request that has already completed, consuming a connection slot. | `main` | | [#369](https://github.com/crtahlin/wasp/issues/369) |

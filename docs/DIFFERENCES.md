...
| Adds a V(2) debug line at an overdraft refusal, `credit refused, would overdraw`, carrying the peer, the chunk price, both sides of the comparison, and each term feeding them: the settled balance, the surplus and the reserved balance. Also logs the shadow reserved balance and the raw refresh timestamp, which bear on the settlement decision rather than on the comparison, and whether the settle branch was entered. Bee increments one unlabelled counter and logs nothing, so a chunk refused for credit and one that does not exist both surface as `storage.ErrNotFound`. Silent unless the `node/accounting` logger is raised to `all`. One further visible difference: a `node/accounting` V(2) row appears in `GET /loggers` from boot, where Bee creates it only after the path has run. | `main` | | [#353](https://github.com/crtahlin/wasp/issues/353) |

...
- **Compared with:** Bee **v2.8.2**, the latest released version of upstream Bee
  on 2026-09-10.
- **wasp described:** commit `69e3d643`, 2026-09-20. The latest wasp release is
  v0.1.3.
...
| Adds `bee_providers_discoveries_started`, `bee_providers_hinted_connects_started`, `bee_providers_lookups_completed`, `bee_providers_lookups_served_from_cache`, `bee_providers_lookups_canceled`, `bee_providers_connects_dialed`, `bee_providers_connects_already_connected` and `bee_providers_connects_failed`. Before these the package had no metrics and no success logging, so a working provider lookup and one that never ran were indistinguishable from outside the node. `connects_failed` excludes a connect abandoned by shutdown or by the run's own timeout, so that one shutdown does not read as many unreachable providers. **`connects_dialed` and `connects_already_connected` are decided by reading the peer set immediately before the connect**, not from what `Connect` returns. They were originally split on `p2p.ErrAlreadyConnected`, which is decided by matching the remote **address** of an open connection rather than the peer, so a provider already connected on a different underlay came back as a plain success and was counted as a dial. A provider record's addresses differ from the address book's by construction, since the record is filtered to public underlays. The connect itself is unchanged: it still runs, still checks the overlay, and still notifies topology. What remains is a narrow race, kademlia connecting the peer between the read and the connect, which counts that connect as a dial; these counters are a diagnostic rather than an accounting record. The underlying address keying is upstream's and is left alone, because changing it would also stop kademlia announcing a peer it re-dials, stop this adapter notifying topology, and turn `POST /connect` from 200 into 500 for an already-connected peer ([#382](https://github.com/crtahlin/wasp/issues/382)). | `main` | | [#369](https://github.com/crtahlin/wasp/issues/369), [#382](https://github.com/crtahlin/wasp/issues/382) |

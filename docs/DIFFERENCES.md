...
| Adds `bee_retrieval_preferred_candidates_selected`, flights that began with at least one preferred candidate, counted once per flight rather than once per attempt. It exists because no existing counter can say whether the preferred set reached a fetch with a usable peer in it: `preferred_attempts` increments only after the peer can be credited, and attempts plus overdrafts is inflated by the readmit path, which keeps a peer without consuming the candidate. It counts a **usable** set: candidates are filtered to connected full nodes that are not in the per-chunk error-skip list, so a set that arrived with every peer disconnected, or whose peer errored on that chunk within the last minute, reads zero. | `main` | | [#299](https://github.com/crtahlin/wasp/issues/299) |

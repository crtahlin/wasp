...
- **Compared with:** Bee **v2.8.2**, the latest released version of upstream Bee
  on 2026-09-10.
- **wasp described:** commit `f5fd2b51`, 2026-09-20. The latest wasp release is
  v0.1.3.
...
| Adds `bee_providers_discoveries_started`, `bee_providers_hinted_connects_started`, `bee_providers_lookups_completed`, `bee_providers_lookups_served_from_cache`, `bee_providers_lookups_canceled`, `bee_providers_connects_dialed`, `bee_providers_connects_already_connected` and `bee_providers_connects_failed`. Before these the package had no metrics and no success logging, so a working provider lookup and one that never ran were indistinguishable from outside the node. `connects_dialed` counts a connect procedure running to completion and does **not** prove a new connection was opened, because libp2p's already-connected short-circuit is keyed on the remote address rather than the peer. `connects_failed` excludes a connect abandoned by shutdown or by the run's own timeout, so that one shutdown does not read as many unreachable providers. | `main` | | [#369](https://github.com/crtahlin/wasp/issues/369) |

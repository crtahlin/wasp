...
// DefaultOptions returns options comparable to what goleveldb gives by default.
//
// The bloom filter is the reason this function exists. goleveldb applies its
// filter policy across the database from one setting; Pebble configures filters
// per level and applies none unless asked. A zero pebble.Options therefore has
// no bloom filter at all, and a lookup for a key that is not present has to
// touch every level.
//
// Measured before this was added: 1,534 ns against goleveldb's 201 ns for
// ReadRandomMissing, 7.6x slower. That is not a property of Pebble; it is the
// cost of comparing a tuned engine against an untuned one. See
// docs/experiments/pebble-evaluation/results.md.

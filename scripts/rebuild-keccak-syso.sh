...
echo "          Patch 0002 raises that from 8192, which was below the blob's"
echo "          measured ceiling of 11484 bytes (times4) and would overflow the"
echo "          goroutine stack by ~3.3 KB — silently, since digests stay"
echo "          correct. See issue #89."
echo ""
echo "          Do NOT copy the generated .s into pkg/keccak even so. Those"
echo "          stubs deliberately use no Go frame at all: they run the blob on"
echo "          a pooled scratch stack, which is the fix for the memory"
echo "          corruption in issue #92. A correctly sized Go frame is still the"
echo "          wrong place for this code. Only the .syso files are taken."

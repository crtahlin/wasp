...
if ! git rev-parse -q --verify "refs/tags/exp/$SLUG" >/dev/null; then
  echo "error: no tag exp/$SLUG" >&2
  echo "known experiments:" >&2
  git tag -l 'exp/*' | sed 's|^exp/|  |' >&2
  exit 1
fi

M="$(git rev-parse "exp/$SLUG^{commit}")"
...
if [ "$(git rev-list --no-walk --count --merges "$M")" -ne 1 ]; then
  echo "error: exp/$SLUG does not point at a merge commit." >&2
